...
type Service[U comparable] struct {
	...
	// userMap maps auth-password -> user value (U). It is swapped wholesale by
	// UpdateUsers at runtime (hot reload) while the auth read path reads it
	// concurrently, so access goes through atomic.Pointer to stay race-free.
	// Already-authenticated sessions cache their user in serverSession.authUser
	// and never re-read this map, so swapping it never disturbs live connections.
	userMap           atomic.Pointer[map[string]U]
	udpDisabled       bool
	udpTimeout        time.Duration
	handler           ServerHandler
	masqueradeHandler http.Handler
...
func NewService[U comparable](options ServiceOptions) (*Service[U], error) {
	...
	initialMap := make(map[string]U)
	service.userMap.Store(&initialMap)
	return service, nil
}
...
func (s *Service[U]) UpdateUsers(userList []U, passwordList []string) {
	...
	// Atomic swap: live sessions are unaffected (they cache authUser); only new
	// handshakes observe the new map. Race-free against the auth read path.
	s.userMap.Store(&userMap)
}
...
func (s *serverSession[U]) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost && r.Host == protocol.URLHost && r.URL.Path == protocol.URLPath {
		...
		user, loaded := (*s.userMap.Load())[request.Auth]
		if !loaded {
