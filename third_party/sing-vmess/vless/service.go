...
// userTable bundles the UUID->user and user->flow maps so UpdateUsers can swap
// both at once. It is held by Service.users as an atomic.Pointer: UpdateUsers
// builds a fresh userTable and stores it wholesale, while the auth read path
// (NewConnection) Loads it once per handshake. Swapping both maps together via
// a single pointer store avoids the "new userMap + old userFlow" tearing window
// that two separate field assignments would expose, and the atomic access keeps
// the hot-reload path race-free without locks. Already-handled connections do
// not re-read this table, so swapping never disturbs live connections.
type userTable[T comparable] struct {
	userMap  map[[16]byte]T
	userFlow map[T]string
}

type Service[T comparable] struct {
	users   atomic.Pointer[userTable[T]]
...
func NewService[T comparable](logger logger.Logger, handler Handler) *Service[T] {
	...
	// Seed an empty table so the auth read path can Load() unconditionally
	// before the first UpdateUsers. Lookups against the empty maps return
	// not-loaded, matching the original nil-map behaviour byte-for-byte.
	s.users.Store(&userTable[T]{
		userMap:  make(map[[16]byte]T),
		userFlow: make(map[T]string),
	})
	return s
}
...
func (s *Service[T]) NewConnection(ctx context.Context, conn net.Conn, source M.Socksaddr, onClose N.CloseHandlerFunc) error {
	...
	users := s.users.Load()
	user, loaded := users.userMap[request.UUID]
	if !loaded {
		return E.New("unknown UUID: ", uuid.FromBytesOrNil(request.UUID[:]))
	}
	ctx = auth.ContextWithUser(ctx, user)
	userFlow := users.userFlow[user]
